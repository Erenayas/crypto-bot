...
Stats replay(const std::string& path) {
    ...
    while (reader.next(line)) {
        ...
        if (kind == "h") {
            ...
            if (!snap) { ++st.undecodable; continue; }
            if (sync.on_snapshot(*snap) == SyncAction::RequestSnapshot) {
                // The recording itself contains the next snapshot, so there is
                // nothing to "request" -- we just keep reading.
            }
        } else if (kind == "e") {
            ++st.events;
            const auto ev = decode_depth_event(payload);
            if (!ev) { ++st.undecodable; continue; }
...
void print(const Stats& st, const std::string& path) {
    ...
    std::printf("replay %s\n", path.c_str());
    std::printf("  meta         %s\n", st.meta.c_str());
    std::printf("  records      %llu  (snapshots %llu, events %llu)\n",
    ...
    if (st.best_bid && st.best_ask) {
        std::printf("  final book   bid %.2f / ask %.2f   levels %zu/%zu\n",
                    to_double(st.best_bid->px), to_double(st.best_ask->px),
                    st.bid_levels, st.ask_levels);
    } else {
